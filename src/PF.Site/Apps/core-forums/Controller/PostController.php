        ...
        if ($this->request()->get('req3') == 'thread') {
            ...
            if ($aCallback === false) {
                $this->template()->setBreadCrumb($aForum['breadcrumb'])
                    ->setBreadCrumb(Phpfox::getSoftPhrase($aForum['name']),
            ...
            $this->template()
                ...
                ->assign(array(
                        ...
                        'sForumParents' => ($aCallback === false ? ((Phpfox::getUserParam('forum.can_post_announcement') || Phpfox::getService('forum.moderate')->hasAccess($aForum['forum_id'],
                                'post_announcement')) ? Phpfox::getService('forum')->active($aForum['forum_id'])->getJumpTool(true) : '') : ''),
                        'bPosted' => $bPosted,

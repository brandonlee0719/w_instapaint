...
class Process extends Phpfox_Service
{
    ...
    public function add($aVals, $aCallback = false, $aExtra = array())
    {
        ...
        $bApprovePost = (Phpfox::getUserParam('forum.approve_forum_post') ? true : false);
    ...
    public function approve($iPostId)
    {
        ...
        $sCurrentUrl = Phpfox_Url::instance()->permalink('forum.thread', $aThread['thread_id'], $aThread['title'],
            false, null, array('view' => $aPost['post_id']));
    ...
    public function delete($iPostId)
    {
        ...
        if (!$iPostCount) {
            ...
            if (isset($aLastThread['thread_id'])) {
                $this->database()->update(Phpfox::getT('forum'),
                    array('thread_id' => $aLastThread['thread_id'], 'post_id' => $aLastThread['post_id']),
                    'thread_id = ' . $aPost['thread_id']);
            }
            ...
            if ($aPost['last_post_id'] == $aPost['post_id']) {
                ...
                if (isset($aLastPost['post_id'])) {
                    $this->database()->update(Phpfox::getT('forum'), array('post_id' => $aLastPost['post_id']),
                        'post_id = ' . $aPost['post_id']);
                    $this->database()->update(Phpfox::getT('forum_thread'), array('post_id' => $aLastPost['post_id']),
                        'thread_id = ' . $aPost['thread_id']);
                }
            }
            $this->database()->update(Phpfox::getT('forum_thread'),array('total_post' => array('= total_post -', 1)),
                'thread_id = ' . $aPost['thread_id']);
        }
    ...
    public function update($iId, $iUserId, $aVals)
    {
        ...
        Phpfox::getService('feed.process')->update('forum_post', $iId);

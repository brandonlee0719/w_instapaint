...
?>
<form class="form" method="post" action="{url link='admincp.pages'}">
    ...
    {_p var='choose_action_to_do_with_child_pages'}
    <div class="form-group">
        <div class="radio">
            <label>
                <input type="radio" name="child_action" value="del" onclick="$('#new_category_id').attr('disabled', true);" checked>
                {if $bIsSub}
                    {_p var="remove_all_pages_belonging_to_this_category"}
                {else}
                    {_p var="remove_all_pages_and_sub_categories_belonging_to_this_category"}
                {/if}
            </label>
        </div>
        <div class="radio">
            <label>
                ...
                {if $bIsSub}
                    {_p var="select_another_category_to_move_all_pages_belonging_to_this_category"}
                {else}
                    {_p var="select_another_category_to_move_all_pages_and_sub_categories_belonging_to_this_category"}
                {/if}
            </label>
        </div>
    </div>
    <div class="form-group">
        ...
        <select name="new_category_id" class="form-control" id="new_category_id" disabled>
            {foreach from=$aAllCategories item=aCategory}
                {if $aCategory.type_id != $iCategoryId || $bIsSub}
    ...
    {else}
    <p class="help-block">{_p var='delete_category_notice_pages'}</p>
    {/if}
    {else}

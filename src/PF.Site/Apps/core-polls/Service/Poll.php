...
class Poll extends \Phpfox_Service
{
    ...
    public function checkStructure($aVals)
    {
        ...
        foreach ($aVals['answer'] as $aAnswer) {
            ...
            if (is_numeric($sAnswer)) {
                $aErrors[1] = _p('we_dont_allow_default_answers_answer', array('answer' => $aAnswer['answer']));
                continue;
            }
        ...
        if ($iTotalPass < 2) {
            Phpfox_Error::set(_p('you_need_to_write_at_least_2_answers'));
        }
    ...
    public function getPollByUrl($sUrl, $iPage = false, $iPageSize = false, $bIsView = false, $bShowAnswer = false)
    {
        ...
        $aPoll = $this->database()->select('p.*,' . (Phpfox::getParam('core.allow_html') ? 'p.description_parsed' : 'p.description') . ' AS description, pd.background, pd.percentage, pd.border, pr.user_id as voted, pr.answer_id, ' . Phpfox::getUserField())
            ->from($this->_sTable, 'p')
        ...
        if ($aPoll['close_time']) {
            ...
            $aPoll['close_time'] = Phpfox::getLib('date')->convertFromGmt($aPoll['close_time'], Phpfox::getTimeZone());
            $aPoll['close_month'] = date('n', $aPoll['close_time']);
            $aPoll['close_day'] = date('j', $aPoll['close_time']);
            $aPoll['close_year'] = date('Y', $aPoll['close_time']);
            $aPoll['close_hour'] = date('H', $aPoll['close_time']);
            $aPoll['close_minute'] = date('i', $aPoll['close_time']);
        }
     ...
     * @deprecated
     * @since 3.0.0beta1
     * @param mixed $aCond , deprecated, remove in 4.7.0
     * @param mixed $iPage Page to show
     * @param mixed $iPageSize How many items per page
     * @param string $sOrder , deprecated, remove in 4.7.0
     * @return array
     */
    public function getPolls($aCond = array(), $iPage = false, $iPageSize = false, $sOrder = null)
    {
        ...
        $aPolls = $this->database()->select('poll.*,  pd.background, pd.percentage, pd.border, pr.answer_id, pr.user_id as voted, friends.friend_id AS is_friend, ' . Phpfox::getUserField())
            ...
            ->leftJoin(Phpfox::getT('poll_result'), 'pr',
                'pr.poll_id = poll.poll_id AND pr.user_id = ' . Phpfox::getUserId())
            ->leftJoin(Phpfox::getT('like'), 'l',
                'l.type_id = \'poll\' AND l.item_id = poll.poll_id AND l.user_id = ' . Phpfox::getUserId())
            ->leftJoin(Phpfox::getT('friend'), 'friends',
        ...
        //If $sPolls empty next query will error
        if (empty($sPolls)) {
            $sPolls = 0;
        }
        $aAnswers = $this->database()->select('pa.*, pr.user_id as voted')
    ...
    public function getVotes($iPollid, $iPage = 0, $iPageSize = 0, &$iCount = 0)
    {
        ...
        $aResult = [];
        if ($iCount) {
            $aUserVotes = db()->select(Phpfox::getUserField())
                ->from(Phpfox::getT('poll_result'), 'pr')
    ...
    public function getPermissions(&$aRow)
    {
        ...
        $aRow['canSponsorInFeed'] = (Phpfox::isModule('ad') && $aRow['user_id'] == Phpfox::getUserId() && Phpfox::isModule('feed') && (Phpfox::getUserParam('feed.can_purchase_sponsor') || Phpfox::getUserParam('feed.can_sponsor_feed')) && (Phpfox::getService('feed')->canSponsoredInFeed('poll',
                $aRow['poll_id'])));
        $aRow['iSponsorInFeedId'] = Phpfox::getService('feed')->canSponsoredInFeed('poll', $aRow['poll_id']);
        $aRow['canSponsor'] = (Phpfox::isModule('ad') && Phpfox::getUserParam('poll.can_sponsor_poll') && $aRow['view_id'] == 0 && $aRow['item_id'] == 0);
        $aRow['canPurchaseSponsor'] = (Phpfox::isModule('ad') && $aRow['is_sponsor'] != 1 && $aRow['user_id'] == Phpfox::getUserId() && Phpfox::getUserParam('poll.can_purchase_sponsor_poll') && $aRow['view_id'] == 0 && $aRow['item_id'] == 0);
    ...
    public function getFeatured($iLimit = 4, $iCacheTime = 5)
    {
        ...
        if (!($sPollIds = $this->cache()->get($sCacheId, $iCacheTime))) {
            $aPollIds = $this->database()->select('p.poll_id')
                ->from($this->_sTable, 'p')
                ->join(Phpfox::getT('user'), 'u', 'u.user_id = p.user_id')
                ->where('p.view_id = 0 AND p.is_featured = 1 AND p.item_id = 0')
                ->order('rand()')
    ...
    public function getSponsored($iLimit = 4, $iCacheTime = 5)
    {
        ...
        if (!($sPollIds = $this->cache()->get($sCacheId, $iCacheTime))) {
            $aPollIds = $this->database()->select('p.poll_id')
                ->from($this->_sTable, 'p')
                ->join(Phpfox::getT('user'), 'u', 'u.user_id = p.user_id')
                ->join(Phpfox::getT('ad_sponsor'), 's', 's.item_id = p.poll_id')
                ->where('p.view_id = 0 AND p.item_id = 0 AND p.is_sponsor = 1 AND s.module_id = \'poll\' AND s.is_active = 1')
                ->order('rand()')
                ->limit(Phpfox::getParam('core.cache_total'))
                ->execute('getSlaveRows');
            foreach ($aPollIds as $key => $aId) {
        ...
        $aPolls = $this->database()->select( Phpfox::getUserField() . ',p.*, p.total_view as total_view_poll, s.* ')
            ->from($this->_sTable, 'p')
            ->join(Phpfox::getT('user'), 'u', 'u.user_id = p.user_id')
            ->join(Phpfox::getT('ad_sponsor'), 's', 's.item_id = p.poll_id AND s.module_id = \'poll\'')
            ->where('p.poll_id IN (' . implode(',', $aPollIds) . ')')
            ->limit($iLimit)
            ->execute('getSlaveRows');

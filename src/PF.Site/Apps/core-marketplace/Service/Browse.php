...
class Browse extends Phpfox_Service
{
    ...
    public function execute__()
    {
        ...
        if ($this->_sCategory !== null) {
            ...
            $this->database()->innerJoin(Phpfox::getT('marketplace_category_data'), 'mcd',
                'mcd.listing_id = m.listing_id');
        ...
        if ($this->_bIsSeen !== false) {
            $this->database()->join(Phpfox::getT('marketplace_invite'), 'mi',
                'mi.listing_id = m.listing_id AND mi.visited_id = 0 AND mi.invited_user_id = ' . Phpfox::getUserId());
        }

        if (Phpfox_Request::instance()->get('view') == 'purchased') {
            $this->database()->join(Phpfox::getT('marketplace_invoice'), 'minvoice',
                'minvoice.listing_id = m.listing_id AND minvoice.user_id = ' . Phpfox::getUserId() . ' AND minvoice.status = \'completed\'');
        }

        if (Phpfox_Request::instance()->get('view') == 'sold') {
            $this->database()->join(Phpfox::getT('marketplace_invoice'), 'minvoice',
                'minvoice.listing_id = m.listing_id AND minvoice.status = \'completed\'');
        }
        ...
        if ($this->_iCnt) {
            if ($this->_sCategory !== null) {
                $this->database()->innerJoin(Phpfox::getT('marketplace_category_data'), 'mcd',
                    'mcd.listing_id = m.listing_id')->group('m.listing_id');
            }
            ...
            if (Phpfox_Request::instance()->get('view') == 'sold') {
                $this->database()->join(Phpfox::getT('marketplace_invoice'), 'minvoice',
                    'minvoice.listing_id = m.listing_id AND minvoice.status = \'completed\'')->group('m.listing_id');
            }
            ...
            if (Phpfox_Request::instance()->get('view') == 'sold') {
                foreach ($this->_aListings as $iKey => $aListing) {
                    list(, $aSoldItems) = Phpfox::getService('marketplace')->getInvoices('mi.listing_id = ' . $aListing['listing_id'] . ' AND mi.status = \'completed\'',
                        true);
...
//            $aRows[$iKey] = $aListing;
            $aRows[$iKey]['aFeed'] = array(
                ...
                'feed_link' => Phpfox_Url::instance()->permalink('marketplace', $aListing['listing_id'],
                    $aListing['title']),
            ...
            $aRows[$iKey]['url'] = Phpfox_Url::instance()->permalink('marketplace', $aListing['listing_id'],
                $aListing['title']);
        }
    }
    ...
    public function query()
    {
        ...
        $this->database()->select('mt.description_parsed AS description, ')->join(Phpfox::getT('marketplace_text'),

...
class Callback extends Phpfox_Service
{
    ...
    {
        if ($aItem['item_id'] == $aItem['user_id']) {
            $aParams = array();
        } else {
            $aParams = array('poke-id' => $aItem['item_id']);
        }
        $aItemUser = Phpfox::getService('user')->getUser($aItem['item_user_id']);
        return array(
            'link' => Phpfox::getLib('url')->makeUrl($aItemUser['user_name'], $aParams),
            'message' => _p('full_name_has_poked_you',
                array('full_name' => '<span class="drop_data_user">' . $aItem['full_name'] . '</span>'))
        );
    }
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => _p('full_name_commented_on_your_poke',
                    array('full_name' => Phpfox::getUserBy('full_name'))),
                'owner_message' => _p('full_name_commented_on_your_poke_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink)),
                'owner_notification' => 'comment.add_new_comment',
                'notify_id' => 'poke_comment',
                'mass_id' => 'poke_comment',
                'mass_subject' => (Phpfox::getUserId() == $aRow['user_id'] ? _p('full_name_commented_on_gender_poke',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1)
                    )) : _p('full_name_commented_on_row_full_name_s_poke',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'row_full_name' => $aRow['full_name']))),
                'mass_message' => (Phpfox::getUserId() == $aRow['user_id'] ? _p('full_name_commented_on_gender_poke_to_see_the_comment_thread',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1),
                        'link' => $sLink
                    )) : _p('full_name_commented_on_row_full_name_s_poke_message', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'row_full_name' => $aRow['full_name'],
                    'link' => $sLink
                )))

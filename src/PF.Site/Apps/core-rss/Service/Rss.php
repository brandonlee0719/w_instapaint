...
class Rss extends Phpfox_Service
{
    /**
     * Class constructor
     */
    ...
    public function output($aParam)
    {
        ...
        $oXmlBuilder->addTag('atom:link', '', array(
            ...
            ->addTag('link', (isset($aParam['link']) ? $aParam['link'] : Phpfox::getLib('url')->makeUrl('current')))
        ...
        foreach ($aParam['items'] as $aItem) {
            ...
            $aItem['description'] = preg_replace_callback('/\[PHPFOX_PHRASE\](.*?)\[\/PHPFOX_PHRASE\]/i',
                array($this, '_getPhrase'), $aItem['description']);
    ...
    public function getForEdit($iId)
    {
        ...
        if (!isset($aFeed['feed_id'])) {
            return Phpfox_Error::set(_p('unable_to_find_the_feed_you_are_planning_to_edit'));
        }

        $aLanguages = Phpfox::getService('language')->getAll();
        foreach ($aLanguages as $aLanguage) {
            $aFeed['title_var_' . $aLanguage['language_id']] = Phpfox::getSoftPhrase($aFeed['title_var'], [], false,
                null, $aLanguage['language_id']);
            $aFeed['description_var_' . $aLanguage['language_id']] = Phpfox::getSoftPhrase($aFeed['description_var'], [], false,
                null, $aLanguage['language_id']);
        }
    ...
    public function getUserFeed(&$aUser)
    {
        ...
        $oXmlBuilder->addTag('atom:link', '', array(
            ...
            ->addTag('description', _p('latest_updates_from_full_name',
                array('full_name' => Phpfox::getLib('parse.output')->clean($aUser['full_name']))));

        $iLog = db()->select('COUNT(*)')
            ->from(Phpfox::getT('rss_log_user'))
            ->where('user_id = ' . $aUser['user_id'] . ' AND id_hash = \'' . Phpfox::getLib('request')->getIdHash() . '\'')
            ->execute('getSlaveField');
        if (!$iLog) {
            db()->insert(Phpfox::getT('rss_log_user'), array(
            ...
            db()->updateCounter('user_field', 'rss_count', 'user_id', $aUser['user_id']);
        }

        if (is_array($aFeeds) && count($aFeeds)) {
            foreach ($aFeeds as $aItem) {
                $oXmlBuilder->addGroup('item');
                $oXmlBuilder->addTag('title',
                    Phpfox::getLib('parse.output')->clean($aItem['full_name']) . ' ' . strip_tags(!empty($aItem['feed_info']) ? $aItem['feed_info'] : (!empty($aItem['feed_content']) ? $aItem['feed_content'] : '')));
    ...
    public function getFeed($iId)
    {
        ...
        if (!($aFeed = $this->cache()->get($sCacheId))) {
            ...
            if (!isset($aFeed['feed_id'])) {
                return Phpfox_Error::set(_p('unable_to_find_rss_feed'));
            }

            $this->cache()->save($sCacheId, $aFeed);
        }

        $sDescription = _p($aFeed['description_var']);

        eval($aFeed['php_view_code']);

        if (!isset($aRows) || (isset($aRows) && !is_array($aRows))) {
            return Phpfox_Error::trigger(_p('not_a_valid_rss_feed_php_code_failed'), E_USER_ERROR);
        }
        ...
        $iLog = db()->select('COUNT(*)')
            ...
            ->execute('getSlaveField');
        if (!$iLog) {
            db()->insert(Phpfox::getT('rss_log'), array(
            ...
            db()->updateCounter('rss', 'total_subscribed', 'feed_id', $aFeed['feed_id']);
        }
        ...
        $oXmlBuilder->addTag('atom:link', '', array(
            ...
            ->addTag('title', html_entity_decode(_p($aFeed['title_var']), null, 'UTF-8'))
        ...
        foreach ($aRows as $aItem) {
            ...
            $oXmlBuilder->addTag('title', $aItem['title']);
            $oXmlBuilder->addTag('link', $aItem['link']);
            $oXmlBuilder->addTag('description',
                Phpfox::getLib('parse.output')->shorten($aItem['description'], 150, '...'));
    ...
    public function getLinks()
    {
        ...
        if (!($aFeeds = $this->cache()->get($sCacheId))) {
            $aRows = db()->select('r.feed_id, r.title_var, r.php_group_code')
                ->from($this->_sTable, 'r')
                ->join(Phpfox::getT('module'), 'm', 'm.module_id = r.module_id AND m.is_active = 1')
                ->join(Phpfox::getT('product'), 'p', 'p.product_id = r.product_id AND p.is_active = 1')
            ...
            foreach ($aRows as $aRow) {
                ...
                if (isset($aRow['child'])) {
                    ...
                    $aFeeds[Phpfox::getLib('url')->makeUrl('rss',
                        array('id' => $aRow['feed_id']))] = _p($aRow['title_var']);
                }
            }
    ...
    public function getFeeds()
    {
        ...
        if (!($aFeeds = $this->cache()->get($sCacheId))) {
            $aGroupRows = db()->select('rg.group_id, rg.name_var')

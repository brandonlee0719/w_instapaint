...
class FrameController extends Phpfox_Component
{
    public function process()
    {
        ...
        $sErrorMessage = '';
        $bNoFeed = false;
        $iUserId = (Phpfox::isModule('pages') && isset($aVals['page_id']) && $aVals['page_id']) ?
            Phpfox::getService('pages')->getUserId($aVals['page_id']) : Phpfox::getUserId();

        if (Phpfox::isModule('pages') && !empty($aVals['page_id'])) {
            $iUserId = Phpfox::getService('pages')->getUserId($aVals['page_id']);
        } elseif (Phpfox::isModule('groups') && !empty($aVals['groups_id'])) {
            ...
            if ($sError == UPLOAD_ERR_OK) {
                if ($aImage = $oFile->load('image[' . $iKey . ']', array(
                    ...
                    if ($iId = $oServicePhotoProcess->add($iUserId, array_merge($aVals, $aImage))) {
                        $iCnt++;
                        $aPhoto = Phpfox::getService('photo')->getForProcess($iId, $iUserId);
                    ...
                    if ($bIsInline && empty($aVals['is_cover_photo'])) {
                        $bUploadFail = true;
                        $sMessage = _p('photo_title', ['name' => $_FILES['image']['name'][$iKey]]);

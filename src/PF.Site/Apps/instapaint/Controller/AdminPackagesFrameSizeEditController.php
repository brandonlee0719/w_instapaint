...
class AdminPackagesFrameSizeEditController extends \Phpfox_Component
{
        ...
        if ($vals) { // Form was sent, edit and redirect
            ...
            if (!empty($vals)) {
                ...
                if (\Phpfox_Error::isPassed()) {
                    $packagesService->updateFrameSize($frameSizeId, $vals['name'], $vals['description'], $vals['price']);
                    $this->url()->send('admin-dashboard/packages',null,'Frame size was successfully updated');
                }
            }
            ...
            // Set title
            $template->setTitle('Admin Dashboard » Edit Frame Size');
            ...
            $template->setBreadCrumb('Edit Frame Size');
            ...
            $template->buildSectionMenu('admin-dashboard', $instapaintService->insertMenuAfter(
                ...
                ['Edit Frame Size' => 'admin-dashboard.packages.frame-size.edit.' . $frameSizeId] // Menu to be inserted
            ));

            // If frame size was not found in DB, let's show an error message:
            if (!$frameSize) {
                $template->assign([
                    'error' => 'The frame size you are trying to edit does not exist.'
                ]);

...
class AdminPaintersSetDailyLimitController extends \Phpfox_Component
{
    public function process()
    {
        ...
        // Set view title
        $template->setTitle('Admin Dashboard » Daily jobs limit');
        ...
        $template->setBreadCrumb('Update daily jobs limit');
        ...
        $template->buildSectionMenu('admin-dashboard', $instapaintService->insertMenuAfter(
            ...
            ['Daily jobs limit' => 'admin-dashboard.painters.set-daily-limit.' . $painterId] // Menu to be inserted
        ));
        ...
        $template->assign([
            ...
            'maxDailyLimit' => $adminService->getMaxDailyOrders($painterId)
        ]);
        ...
        if ($vals) { // Form was sent
            ...
            if (\Phpfox_Error::isPassed()) {
                ...
                $adminService->setPainterDailyJobsLimit($painterId, (int) $vals['daily_limit']);
                $this->url()->send('/admin-dashboard/painters/approved/',null,'Your changes were saved!');
            }

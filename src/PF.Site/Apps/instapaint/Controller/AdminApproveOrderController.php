...
class AdminApproveOrderController extends \Phpfox_Component
{
    public function process()
    {
        ...
        if ($order && (int) $order['order_status_id'] == 2 && !$order['is_shipped']) {
            ...
            if ($vals) { // Form was sent
                ...
                if (\Phpfox_Error::isPassed()) {
                    ...
                    $services['admin']->approveOrder($orderId);
                    \Phpfox::getService('notification.process')->add('instapaint_PainterOrderApprovedForShipping', $orderId, $order['painter_user_id'], $order['painter_user_id'], true);
                    $this->url()->send('/admin-dashboard/verify-orders/', null, 'The order was approved!', null, 'success');
                }
        ...
        // Set view title
        $template->setTitle('Admin Dashboard » Approve Order');
        ...
        $template->setBreadCrumb('Approve Order');
        ...
        $template->buildSectionMenu('admin-dashboard', $services['instapaint']->insertMenuAfter(
            ...
            ['Approve Order' => 'admin-dashboard.approve-order.' . $orderId] // Menu to be inserted
        ));

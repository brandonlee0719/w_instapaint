...
class AdminPaintersApprovalRequestDenyController extends \Phpfox_Component
{
    public function process()
    {
        ...
        if ($vals) { // Form was sent
            ...
            if (\Phpfox_Error::isPassed()) {
                ...
                if ($approvalRequest) {
                    if ($painterService->denyApprovalRequest($approvalRequest)) {
                        $this->url()->send('admin-dashboard/painters/approval-requests',null,'The approval request was denied successfully');
                    } else {
                        $this->url()->send('admin-dashboard/painters/approval-requests',null,'There was an error denying the approval request', null, 'danger');
                    }

                } else {
                    // If no approval request found, show error in template
                    $this->template()->assign([
                        'error' => 'The approval request you are trying to deny does not exist.'
                    ]);
                }

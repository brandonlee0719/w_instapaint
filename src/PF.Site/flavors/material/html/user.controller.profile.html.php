...
{literal}
<style>
    /* Hide client type field group */
    .form .js_custom_group_basic:nth-child(2) .js_custom_group_1:nth-child(2) {
        display: none;
    }
</style>
{/literal}
...
<form method="post" class="form" action="{url link='user.profile'}"{if !$bIsEdit} onsubmit="{plugin call='user.template_controller_profile_form_onsubmit'} $Core.processing(); $(this).ajaxCall('custom.updateFields'); return false;"{/if}>
    ...
    <div class="js_custom_groups js_custom_group_basic">
        ...
        {foreach from=$aSettings item=aSetting}
            ...
            <div class="form-group js_custom_groups js_custom_group_{$aSetting.group_id}">
...
{literal}
<script>
    // Set a default value for "user type" field since it's required in order to send form
    if (window.document.getElementById('custom_field_2').value === '') {
        window.document.getElementById('custom_field_2').value = 1;
    }
</script>
{/literal}
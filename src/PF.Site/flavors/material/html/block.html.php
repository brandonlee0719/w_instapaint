...
{if (isset($sHeader) && (!PHPFOX_IS_AJAX || isset($bPassOverAjaxCall) || isset($bIsAjaxLoader))) || (defined("PHPFOX_IN_DESIGN_MODE") && PHPFOX_IN_DESIGN_MODE) || (Theme_Service_Theme::instance()->isInDnDMode())}
	...
	<div class="bottom">
		{if count($aFooter) == 1}
			...
			{/if}
            {if is_array($sLink)}
            <a class="btn btn-block {if !empty($sLink.class)} {$sLink.class}{/if}" href="{if !empty($sLink.link)}{$sLink.link}{else}#{/if}" {if !empty($sLink.attr)}{$sLink.attr}{/if} id="js_block_bottom_link_{$phpfox.iteration.block}">{$sPhrase}</a>
            {else}
            <a class="btn btn-block" href="{$sLink}" id="js_block_bottom_link_{$phpfox.iteration.block}">{$sPhrase}</a>
            {/if}
			{/foreach}
		{else}
		<ul>

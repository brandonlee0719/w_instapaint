...
class Phpfox_Date
{
    ...
    public function mktime($iHour, $iMinute, $iSecond, $iMonth, $iDay, $iYear)
    {
        ...
        if ($iYear >= 1970) {
            ...
            $iDate += $iDay - 1;
            $iDate = (($iDate * 86400) + ($iHour * 3600) + ($iMinute * 60)
                + $iSecond);
        } else {
            ...
            $iDate = -(($iDate * 86400) + (86400 - (($iHour * 3600) + ($iMinute
                            * 60) + $iSecond)));
    ...
    {
        return ($this->mktime(0, 0, 0, Phpfox::getTime('m'), 1,
                Phpfox::getTime('Y')) - 3500);
    }
    ...
    public function convertFromGmt($iTime, $sOffset)
    {
        ...
        return (int)(!empty($sOffset) ? (substr($sOffset, 0, 1) == '-' ? ($iTime
            - (substr($sOffset, 1) * 3600)) : $iTime + ($sOffset * 3600))
            : $iTime);
    }
    ...
    public function convertToGmt($iTime)
    {
        ...
        return (int)(!empty($sUserOffSet) ? (substr($sUserOffSet, 0, 1) == '-'
            ? ($iTime + (int)(substr($sUserOffSet, 1) * 3600))
            : $iTime - ($sUserOffSet * 3600)) : $iTime);
    }

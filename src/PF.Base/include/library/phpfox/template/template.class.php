...
class Phpfox_Template
{
	...
	public function checkBundles(){
	    ...
	    if(!($this->_aBundleScripts = $cache->get($sId))){
            ...
            {
                if(is_string($mValue)){
                    $this->_aBundleScripts[] =  $this->_getBundleScript($mKey, $mValue);
                }elseif(is_array($mValue)){
                    foreach($mValue as $key=>$value){
                        $this->_aBundleScripts[] =  $this->_getBundleScript($key, $value);
                    }
                }
            }

            $homeUrl = Phpfox::getLib('cdn')->getUrl(home());

            $this->_aBundleScripts =  array_map(function($file) use($homeUrl){
                return resolve_path(PHPFOX_PARENT_DIR . explode('?', str_replace($homeUrl, '', $file))[0]);
            }, $this->_aBundleScripts);

            $cache->save($sId, $this->_aBundleScripts);
    ...
    private function _getBundleScript($mKey,$mValue){
        ...
        switch ($mValue)
        {
            ...
            case 'static_script':
                return Phpfox::getParam('core.url_static_script'). $mKey;
            case 'flavor_bootstrap':
                $Theme = $this->_theme->get();
                return $basePath .'/PF.Site/flavors/' . flavor()->active->id . '/flavor/' . $Theme->flavor_folder . '.css';
            case 'flavor_boot':
                return $basePath .'/PF.Base/less/version/boot.css';
            default:
                if(preg_match('/app_/i', $mValue)){
                    $aParts = explode('_', $mValue, 2);
                    if($mKey == 'autoload.css'){
                        return $basePath  . '/PF.Site/Apps/'. $aParts[1] . '/assets/' . $mKey;
                    }elseif($mKey == 'autoload.js'){
                        return $basePath  . '/PF.Site/Apps/'. $aParts[1] . '/assets/' . $mKey;
                    }elseif ('.js' == substr($mKey, -3)){
                        return $basePath  . '/PF.Site/Apps/'. $aParts[1] . '/assets/' . $mKey;

                    }elseif('.css' == substr($mKey, -4)){
                ...
                }elseif(preg_match('/flavors_/i', $mValue)){
                    $aParts = explode('_', $mValue, 2);
                    if($mKey == 'autoload.css'){
                        return $basePath  . '/PF.Site/flavors/'. $aParts[1] . '/assets/' . $mKey;
                    }elseif($mKey == 'autoload.js'){
                        return $basePath  . '/PF.Site/flavors/'. $aParts[1] . '/assets/' . $mKey;
                    }elseif ('.js' == substr($mKey, -3)){
                        return $basePath  . '/PF.Site/flavors/'. $aParts[1] . '/assets/' . $mKey;
                ...
                }elseif (preg_match('/module_/i', $mValue)) {
                    $aParts = explode('_', $mValue);
                    if (isset($aParts[1]) && Phpfox::isModule($aParts[1]))
                    {
                        if (substr($mKey, -3) == '.js')
                        {
                            return Phpfox::getParam('core.path_file') . 'module/' . $aParts[1] . '/static/jscript/' . $mKey;
                        }
                        elseif (substr($mKey, -4) == '.css')
                        {
                            $Theme = $this->theme()->get();
                            return $Theme->getCssFileName(str_replace(Phpfox::getParam('core.path_file'),'',$this->getStyle('css', $mKey, $aParts[1])));
                        }
                    }
                }
        }
    ...
    public function getBundleFiles()
    {
        ...
        $homeUrl  =  Phpfox::getLib('cdn')->getUrl(home());
        return array_map(function($file) use($homeUrl){
...
// You are filtering out the controllers which should not load 'content' ajaxly, finding a way for pages.view/1/info and like that
			...
			$aJsVars = array(
					...
					'sJsStatic' => str_replace('/index.php','', $oUrl->getDomain()) .'PF.Base/' . PHPFOX_STATIC,
		...
		if ($bundle) {
            ...
            foreach($this->_aBundleScripts as $key=>$value){
                if(strpos( $value,'.css')){
                    $files[] =  $value;
                }
            }
			$callback = function ($file = null) use(&$files, &$flavor, &$aExcludeCss, $homeUrl){
				if ($file == 'flavor') {
					return $flavor;
				}
                ...
                $the_file = explode('?', (strpos($file, $homeUrl) === false ? $file : (PHPFOX_PARENT_DIR . str_replace($homeUrl, '', $file))))[0];
                
                if(!in_array($the_file, $files)){
                    $aExcludeCss[] =  str_replace(PHPFOX_ROOT,'', $the_file);
                }
			};
			...
			if (!file_exists($path)) {
                foreach ($files as $key => $file) {
                    foreach ($aExcludeCss as $sExcludeCss) {
                        if (strpos($file, $sExcludeCss)) {
                            unset($files[$key]);
                            continue 2;
                        }
                    }
...
//			    ob_get_clean();
//            }
//            var_export($aExcludeCss);exit;
			$sData .= '<link href="' . $homeUrl . 'PF.Base/file/static/' . $the_name . '?v=' . $this->getStaticVersion() . '" rel="stylesheet" type="text/css">';
			foreach ($aExcludeCss as $sCss) {
                $sData .= '<link href="'. $homeUrl. $sCss . '" rel="stylesheet" type="text/css">';
            }
		}
	...
	public function addScript($key, $value, $bReturnString = true){
        ...
        switch ($value)
        {
            ...
            case 'static_script':
                if($bReturnString){
                    return '<script src="'.Phpfox::getParam('core.url_static_script'). $key.'"></script>';
                }
                $this->_sAddScripts .= '<script src="'.Phpfox::getParam('core.url_static_script'). $key.'"></script>';
                break;
            default:
                if(preg_match('/app_/i', $value)){
                    ...
                    if ('.js' == substr($key, -3)){
                        if($bReturnString){
                            return '<script type="text/javascript" src="'.$basePath  . '/PF.Site/Apps/'. $aParts[1] . '/assets/' . $key . '"></script>';
                        }
                        $this->_sAddScripts  .= '<script type="text/javascript" src="'.$basePath  . '/PF.Site/Apps/'. $aParts[1] . '/assets/' . $key . '"></script>';

                    }elseif('.css' == substr($key, -4)){
                        $Theme = $this->theme()->get();
                        $sFileName =  trim(str_replace(dirname(Phpfox::getParam('core.path_file')),'',$this->getStyle('css', $key, $aParts[1],true)),'/');
                        $sFileName = $Theme->getCssFileName($sFileName,'app');

                        if($bReturnString){
                            return '<link type="text/css" rel="stylesheet" href="'. $sFileName .'"/>';
                        }
                        $this->_sAddScripts  .= '<link type="text/css" rel="stylesheet" href="'. $sFileName .'"/>';
                    }
                }elseif (preg_match('/module_/i', $value)) {
                    ...
                    if (isset($aParts[1]) && Phpfox::isModule($aParts[1]))
                    {
                        ...
                        {
                            if($bReturnString){
                                return '<script src="'.Phpfox::getParam('core.path_file') . 'module/' . $aParts[1] . '/static/jscript/' . $key.'"></script>';
                            }
                            $this->_sAddScripts  .= '<script src="'.Phpfox::getParam('core.path_file') . 'module/' . $aParts[1] . '/static/jscript/' . $key.'"></script>';

                        }
                        elseif (substr($key, -4) == '.css')
                        {
                            $Theme = $this->theme()->get();
                            $sFileName = $Theme->getCssFileName(str_replace(Phpfox::getParam('core.path_file'),'',$this->getStyle('css', $key, $aParts[1])));
	...
	public function getFooter($bReturnArray = false) {
		...
		if ($bundle) {
            ...
            $aBundleScripts = [];
            $homeUrl  = Phpfox::getLib('cdn')->getUrl(home());
            $callback = function ($file = null) use ($the_controller, &$aBundleScripts, &$module_files) {
                $script = $file[0];
                $file = $file[1];
                ...
                $the_file = resolve_path(PHPFOX_PARENT_DIR . $the_file);
            ...
            if (!file_exists($path)) {
                $aBundleScripts = array_filter($this->_aBundleScripts,function($file){
                    return strpos($file,'.js')  > 0;
                });
                Phpfox_File_Minimize::instance()->js($path, $aBundleScripts);
            }

            // used to debug.
            if(false){
                Phpfox::getLog('bundle.log')->info([
                    'bundles_scripts'=> $this->_aBundleScripts,
                    'extra_files'=> $module_files,
                ]);
            }

            // do not combine module file, it's cause many problem about conflict.
//            if ($module_files && !file_exists($module_path)) {
//                Phpfox_File_Minimize::instance()->js($module_path, $module_files);
//            }
            $module_urls =  array_map($fnPathToUrl, $module_files);

            $this->_sFooter = str_replace('<!-- autoload -->', '<script src="' . $homeUrl . 'PF.Base/file/static/' . $the_name . '?v=' . $this->getStaticVersion() . '"></script>', $this->_sFooter);
	...
	public function getMenu($sConnection = null)
	{
		...
		if ($sConnection == 'main' && Phpfox::isUser())
		{
			...
			if (($aUserMenusCache = Phpfox::getLib('cache')->get($sUserMenuCache)) === false)
			{
		...
		foreach ($aMenus as $iKey => $aMenu)
		{
            ...
            if ($sPlugin = Phpfox_Plugin::get('template_template_getmenu_process_menu')){eval($sPlugin);}
		}
...
//					$this->setBreadCrumb(preg_replace('/<span(.*)>(.*)<\/span>/i', '', $sMenuName), Phpfox_Url::instance()->makeUrl($sMenuLink), true);
	...
	private function _getCachedName($sName)
	{
		...
		return (defined('PHPFOX_TMP_DIR') ? PHPFOX_TMP_DIR : PHPFOX_DIR_CACHE) . ((defined('PHPFOX_TMP_DIR') || PHPFOX_SAFE_MODE) ? 'template_' : 'template' . PHPFOX_DS) . str_replace(array(PHPFOX_DIR_SITE, PHPFOX_DIR_SITE_APPS, PHPFOX_DIR_THEME, PHPFOX_DIR_MODULE, '/', '\\'), array('', '', '', '', '_', '_'), $sName) . (Phpfox::isAdminPanel() ? '_admincp' : '') . (PHPFOX_IS_AJAX ? '_ajax' : '') . '.php';
	}

			...
			foreach ($mentions as $user) {
                ...
                if ($bSendMail) {
                    Phpfox_Mail::instance()->to($user->id)
                        ->subject($sTagger . ' ' . _p('mentioned_you_in_a_post'))
                        ->message($sTagger . ' ' . _p('mentioned_you_in_a_post') . ' <a href="' . $link . '">' . _p('check_it_out') . '</a>')
                        ->send();
                }
			}
		}

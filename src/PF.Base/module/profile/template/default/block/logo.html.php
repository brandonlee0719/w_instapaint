	...
	<div class="cover_photo_change">
		...
		<form class="form" method="post" action="#">
			<ul class="table_clear_button">
				...
				<li class="js_cover_update_li"><div><input type="button" class="btn button_off" value="{_p var='cancel_cover_photo'}" name="cancel" onclick="window.location.href='{if $bIsPages}{$sPagesLink}{else}{url link='profile'}{/if}';" /></div></li>
				<li class="js_cover_update_li"><div><input type="button" class="btn" value="{_p var='save_changes'}" name="save" onclick="$('.js_cover_update_li').hide(); $('#js_cover_update_loader_upload').show(); $.ajaxCall('{$sAjaxModule}.updateCoverPosition', 'position=' + sCoverPosition{if $sAjaxModule == 'pages'} + '&page_id={$aPage.page_id}'{/if}); return false;" /></div></li>
			</ul>
			<div class="clear"></div>
		</form>
	</div>

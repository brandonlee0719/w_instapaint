...
class Log_Service_Session extends Phpfox_Service
{
	...
	{
	    if(isset($_SERVER['REQUEST_URI']) and $_SERVER['REQUEST_METHOD'] == 'GET' and strpos($_SERVER['REQUEST_URI'],'captcha/image') == false){ // do not update session in ajax mode.
	        return false;
        }
...
//		if (Phpfox::getParam('core.log_site_activity') and Phpfox::getUserId() > 0)
//        {
...
//			}
		...
		if (Phpfox::getParam('core.store_only_users_in_session') and Phpfox::isUser())
		{
...
//				}
			}elseif (!Phpfox::getParam('user.disable_store_last_user')) {
				$update = ['last_activity' => PHPFOX_TIME, 'last_ip_address' => Phpfox::getIp()];

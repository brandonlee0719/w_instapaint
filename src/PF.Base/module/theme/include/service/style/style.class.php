...
class Theme_Service_Style_Style extends Phpfox_Service
{
	...
	public function getFile($iStyleId, $sFileName, $sModule, $bIncludeParent = true)
	{
		...
		{
			return Phpfox_Error::set(_p('unable_to_find_style_sheet'));
		}
		...
		{		
			...
			if (!file_exists($sFile))
			{
				return Phpfox_Error::set(_p('unable_to_find_the_style_sheet_file'));
			}
	...
	{	
		...
		{	
			...
			{
				Phpfox_File::instance()->write($sThemePath . 'phpfox.gif', fox_get_contents($sLogoPath));
			}
			...
			{						
				...
				{							
					...
					Phpfox_File::instance()->write(PHPFOX_DIR_CACHE . $sDirectoryId . PHPFOX_DS . $sDirPath . PHPFOX_DS . $aNewImage['name'], fox_get_contents($aNewImage['file']));							}
			}			
			...
			{						
				...
				Phpfox_File::instance()->mkdir(PHPFOX_DIR_CACHE . $sDirectoryId . PHPFOX_DS . $sDirPath . PHPFOX_DS, true);
				Phpfox_File::instance()->write(PHPFOX_DIR_CACHE . $sDirectoryId . PHPFOX_DS . $sDirPath . PHPFOX_DS . $aScript['name'], fox_get_contents($aScript['file']));
			}			
            ...
            {
                Phpfox_File::instance()->write($sThemePath . 'php' . PHPFOX_DS . 'header.php', fox_get_contents($sPhpHeaderFile));
            }
            ...
            {
            	Phpfox_File::instance()->write($sThemePath . 'sample.png', fox_get_contents($sSamplePngFile));
            }            
		...
		if ($bMultiple === false)
		{
			return Phpfox_Error::set(_p('not_a_valid_style'));
		}
	}
	...
	public function getNewStyles()
	{
		...
		while ($sFolder = readdir($hDir))
		{
			...
			while ($sStyleFolder = readdir($hStyleDir))
			{
				...
				if (!$iInstalled)
				{
					$aStyles[] = Phpfox::getLib('xml.parser')->parse(fox_get_contents($sDir . $sFolder . PHPFOX_DS . 'style' . PHPFOX_DS . $sStyleFolder . PHPFOX_DS . 'phpfox.xml'));
				}
			}
			closedir($hStyleDir);
		}

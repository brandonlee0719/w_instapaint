...
/**
 * Class Ad_Component_Controller_Sponsor
 */
class Ad_Component_Controller_Sponsor extends Phpfox_Component
{
     ...
     * when getInt(add) the user is reviewing a campaign -> dont show the submit
    ...
    public function process()
    {
        ...
        if (($iView = $this->request()->getInt('view')) != 0) {
            ...
            if (empty($aUrl)) {
                Phpfox::addMessage(_p('item_not_found'));
                $this->url()->send('/');
            }
            ...
            if (Phpfox::hasCallback($sModule, 'getLink')) {
                $sLink = Phpfox::callback($sModule . '.getLink',
                    array('item_id' => $aUrl['item_id'], 'section' => $sSection));
                ...
                if (!Phpfox::isAdmin()) {
                    Phpfox_Database::instance()->update(Phpfox::getT('ad_sponsor'),
                        array('total_click' => $aUrl['total_click'] + 1),
                        'sponsor_id = ' . $aUrl['sponsor_id'] . ' AND user_id != ' . Phpfox::getUserId());
                }
                $this->url()->send($sLink);
            }
        }
        ...
        if ($this->request()->get('where') == 'feed') {
            ...
            $aNewItemId = Phpfox::getService('feed')->getForItem($this->request()->get('section'),
                $this->request()->getInt('item'));
            if (!empty($aNewItemId)) {
            ...
            if (Phpfox::getUserParam('feed.can_sponsor_feed')) {
                ...
                // add the sponsor
                Phpfox::getService('ad.process')->addSponsor($aVals);
            }
            // END
        ...
        if (Phpfox::hasCallback($sModule, $sFunction)) {
            ...
            if (strpos($sModule, '_') !== false) {
                ...
                $aPrices = Phpfox::getUserParam($aModule[0] . '.' . $sModule . '_sponsor_price');
            } else {
            ...
            if (is_array($aPrices)) {
                ...
                $aItem['default_cost'] = $aItem['ad_cost'] = $aPrices;
            } else {
        ...
        if (($aVals = $this->request()->getArray('val')) || $iSponsorId > 0) {
            ...
            if (!isset($aVals['total_view']) || ($aVals['total_view'] != 0 && $aVals['total_view'] < 1000)) {
                Phpfox_Error::set(_p('impressions_cant_be_less_than_a_thousand'));
            }
            ...
            if (Phpfox_Error::isPassed()) {
                ...
                if ($iSponsorId < 1) {
                    ...
                    $iInvoiceId = Phpfox::getService('ad.process')->addSponsor($aVals);
                } else {
                    $aInvoice = Phpfox::getService('ad')->getInvoiceSponsor($iSponsorId);
                    $iInvoiceId = !empty($aInvoice) ? $aInvoice['invoice_id'] : 0;
                }
                ...
                $this->setParam('gateway_data', array(
                        ...
                        'amount' => $fTotalCost,
                        'item_name' => $aItem['paypal_msg'],
                        // this is for paypal
                        'return' => $this->url()->makeUrl('ad.invoice', ['sponsor' => 1, 'invoice_id' => $iInvoiceId]),
                        // dummy page to say it all went fine
                        'recurring' => 0,
                        'recurring_cost' => '',
                        'alternative_cost' => 0,
        ...
        $this->template()->setTitle(_p('creating_an_ad'))
            ...
            ->setPhrase(array(
                    'impressions_cant_be_less_than_a_thousand',
                    'the_currency_for_your_membership_has_no_price'
                )
            )
            ->assign(array(
                    'sStatus' => $sStatus,

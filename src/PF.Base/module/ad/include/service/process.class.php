...
class Ad_Service_Process extends Phpfox_Service
{
    ...
    public function addSponsor($aVals)
    {
        ...
        if (strpos($aVals['module'], '_')) {
            $aModule = explode('_', $aVals['module'], 2);
            $sModule = $aModule[0];
            $sSection = $aModule[1];
        } else {
            $sModule = $aVals['module'];
            if (isset($aVals['section'])) {
                $sSection = $aVals['section'];
            }
        }
        $sParam = $sModule . '.can_sponsor_' . (isset($sSection) ? $sSection : $sModule);
        ...
        if (Phpfox::getUserParam($sParam)) {
            ...
            $aPrices = Phpfox::getUserParam($sModule . '.' . $sModule . (isset($sSection) ? "_$sSection" : '') . '_sponsor_price');
        ...
        $iAutoPublish = Phpfox::getUserParam($sModule . '.auto_publish_sponsored_' . (isset($sSection) ? $sSection : 'item'));
        ...
        $aInsertSponsor = array(
            'module_id' => $aVals['module'] . (isset($aVals['section']) && !empty($aVals['section']) ? '_' . $aVals['section'] : ''),
            ...
            'cpm' => $iCpm,
            'start_date' => Phpfox::getUserParam($sParam) ? PHPFOX_TIME : Phpfox::getLib('date')->mktime($aVals['start_hour'],
    ...
    public function updateActivity($aVals)
    {
        ...
        foreach ($aVals as $iId => $aVal) {
            $this->database()->update($this->_sTable, array('is_active' => (isset($aVal['is_active']) ? '1' : '0')),
                'ad_id = ' . (int)$iId);
        }
    ...
    public function update($iId, $aVals)
    {
        ...
        $aSql = array(
            ...
            'end_date' => ($aVals['end_option'] > 0 ? Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->mktime($aVals['end_hour'],
    ...
    public function deleteSponsor($iId, $bByFeedId = false)
    {
        if ($bByFeedId) {
            ...
            $aSponsorItem = db()->select('module_id, item_id')
                ->from(':ad_sponsor')
                ->where('sponsor_id = ' . (int)$iId)
                ->executeRow();
            if (Phpfox::hasCallback($aSponsorItem['module_id'], 'deleteSponsorItem')) {
                Phpfox::callback($aSponsorItem['module_id'] . '.deleteSponsorItem',
                    ['item_id' => $aSponsorItem['item_id']]);
            }
        }
    ...
    public function add($aVals)
    {
        ...
        $iStartTime = Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->mktime($aVals['start_hour'],
            $aVals['start_minute'], 0, $aVals['start_month'], $aVals['start_day'], $aVals['start_year']));
        $iEndTime = ($aVals['end_option'] > 0 ? Phpfox::getLib('date')->convertToGmt(Phpfox::getLib('date')->mktime($aVals['end_hour'],
            $aVals['end_minute'], 0, $aVals['end_month'], $aVals['end_day'], $aVals['end_year'])) : 0);
    ...
    public function addSponsorViewsCount($iSponsorId, $sModule, $sFunction = 'sponsor')
    {
        ...
        if ((int)$aItem['impressions'] !== 0 && $aItem['impressions'] <= $aItem['total_view']) {
            ...
            $this->database()->update(Phpfox::getT('ad_sponsor'), ['is_active' => '4'],
                'sponsor_id = ' . (int)$iSponsorId);
            return Phpfox::getService($sModule . '.process')->$sFunction($aItem['item_id'], 0);
        }

        $this->database()->update(Phpfox::getT('ad_sponsor'), array('total_view' => $aItem['total_view'] + 1),
            'sponsor_id = ' . (int)$iSponsorId);
        return null;
    }
    ...
    */
        ...
        if (Phpfox::getParam('ad.advanced_ad_filters') && isset($aVals['country_iso_custom']) && is_array($aVals['country_iso_custom']) && !empty($aVals['country_iso_custom']) && isset($aVals['child_country']) && is_array($aVals['child_country']) && !empty($aVals['child_country'])) {
            ...
            foreach ($aValid as $aChecked) {
                if (!isset($aToInsert[$aChecked['country_iso']])) {
                    $aToInsert[$aChecked['child_id']] = array(
                        'ad_id' => $iId,
                        'country_id' => $aChecked['country_iso'],
                        'child_id' => $aChecked['child_id']
                    );
            ...
            if (isset($aVals['country_iso_custom']) && is_array($aVals['country_iso_custom']) && !empty($aVals['country_iso_custom'])) {
                ...
                foreach ($aVals['country_iso_custom'] as $sCountry) {
                    $this->database()->insert(Phpfox::getT('ad_country'),
                        array('ad_id' => $iId, 'country_id' => $sCountry));
                }
                $aVals['country_iso_custom'] = '';
            } else {
    ...
    public function addCustom($aVals)
    {
        ...
        */
        ...
        $iStatus = Phpfox::getUserParam('ad.ad_campaigns_must_be_approved_first') ? 2 : 3;

        $aInsert = array(
            'is_custom' => ($fPrice == 0 ? $iStatus : '1'), // if its free set it as approved
            'user_id' => Phpfox::getUserId(),
            ...
            'is_active' => (($fPrice == 0 && !Phpfox::getUserParam('ad.ad_campaigns_must_be_approved_first')) ? '1' : '0'),
            'location' => $aVals['location'],
            'country_iso' => (empty($aVals['country_iso']) ? null : $aVals['country_iso']),
            'gender' => (empty($aVals['gender']) ? 0 : (int)$aVals['gender']),
            'age_from' => (empty($aVals['age_from']) ? 0 : (int)$aVals['age_from']),
        ...
        if (Phpfox::getParam('ad.advanced_ad_filters')) {
            ...
            if (empty($aVals['city_location'])) {
                ...
                $aInsert['city_location'] = json_encode($aSql['city_location']);
            }
        }
        ...
        if ($aVals['type_id'] == '1') {
            ...
            if (Phpfox::getParam('ad.multi_ad') != true) {
                $aVals['html_code'] = str_replace('#ad-link"',
                    Phpfox_Url::instance()->makeUrl('ad', array('id' => $iId)) . '" target="_blank"',
                    $aVals['html_code']);
                $aVals['html_code'] = preg_replace('/id="js_sample_ad_form_(.*)_(.*)" class="js_ad_holder"/i', '',
                    $aVals['html_code']);
    ...
    public function approve($iId)
    {
        ...
        //Send notification
        $iOwnerUserId = $this->database()->select('user_id')
            ->from(':ad')
            ->where('ad_id=' . (int)$iId)
            ->executeField();
    ...
    {
        $aItem = $this->database()->select('module_id, item_id, user_id')
            ->from(Phpfox::getT('ad_sponsor'))
            ->where('sponsor_id = ' . (int)$iId)
            ->execute('getSlaveRow');
        ...
        // stop showing this sponsor by updating its table
        // this overrides security checks
        ...
        $this->database()->update(Phpfox::getT('ad_sponsor'), [
            'is_custom' => '3',
            'is_active' => '1'
        ], 'sponsor_id = ' . (int)$iId);
        ...
        // send notification
        Phpfox::getService('notification.process')->add('ad_approve_sponsor', $iId, $aItem['user_id'],
            $aItem['user_id']);
    ...
    {
        $aItem = $this->database()->select('user_id')
            ->from(Phpfox::getT('ad_sponsor'))
            ->where('sponsor_id = ' . (int)$iId)
            ->execute('getSlaveRow');
    ...
    public function sendApproveEmail($iId, $bSponsor = false)
    {
        ...
        if ($bSponsor === true) {
            ...
            $sMessage = array(
                'ad.your_sponsor_ad_on_site_name_has_been_approved',
                array(
            ...
            $sMessage = array(
                'ad.your_ad_on_site_name_has_been_approved',
                array(
                    'site_name' => Phpfox::getParam('core.site_title'),
    ...
    public function sendDenyEmail($iId, $bSponsor = false)
    {
        ...
        if ($bSponsor === true) {
            ...
            $sMessage = array(
                'ad.your_sponsor_ad_on_site_name_has_been_denied',
                array(
                    'site_name' => Phpfox::getParam('core.site_title'),
                    'link' => Phpfox_Url::instance()->makeUrl('ad.sponsor', array('view' => $aAd['sponsor_id']))
                )
            ...
            $sMessage = array(
                'ad.your_ad_on_site_name_has_been_denied',
                array(
                    'site_name' => Phpfox::getParam('core.site_title'),
                    'link' => Phpfox_Url::instance()->makeUrl('ad.add', array('id' => $aAd['ad_id']))
                )
    ...
    public function updateActivityAjax($iId, $iType, $iUserId = null)
    {
        ...
        if ($iUserId === null) {
            ...
            if ($aAd['is_active'] != $iType) {
                $this->database()->update($this->_sTable, array('is_active' => (int)($iType == '1' ? 1 : 0)),
                    'ad_id = ' . (int)$iId . ' AND user_id = ' . (int)$iUserId);
            }
        }
        ...
        if ($bIsOwner || Phpfox::isAdmin()) {
            if ($iType == '1') {
                ...
                Phpfox::callback($sModule . '.enableSponsor',
                    array('item_id' => $aAd['item_id'], 'section' => $sSection));
            }

            $this->database()->update(Phpfox::getT('ad_sponsor'), array('is_active' => (int)($iType == '1' ? 1 : 0)),
                'sponsor_id = ' . (int)$iId);

            Phpfox::getLib('cache')->removeGroup('ad');
            Phpfox::getLib('cache')->removeGroup('event');
    ...
    public function updateAdPlacementActivity($iId, $iType)
    {
        Phpfox::isUser(true);
        Phpfox::getUserParam('admincp.has_admin_access', true);
        $this->database()->update(Phpfox::getT('ad_plan'), array('is_active' => (int)($iType == '1' ? 1 : 0)),
            'plan_id = ' . (int)$iId);
        Phpfox::getLib('cache')->removeGroup('ad');
    }
    ...
    public function deleteAdminSponsor($sModule, $iItem)
    {
        ...
        $this->database()->delete(Phpfox::getT('ad_sponsor'),

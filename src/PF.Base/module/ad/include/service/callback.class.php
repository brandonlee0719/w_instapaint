...
class Ad_Service_Callback extends Phpfox_Service
{
    ...
    public function paymentApiCallback($aParams)
    {
        ...
        $this->database()->update(Phpfox::getT('ad_invoice'), [
        ...
        if (isset($aAd['auto_publish'])) {
            ...
            $this->database()->update(Phpfox::getT('ad_sponsor'), [
                'is_custom' => $aAd['auto_publish'] == 1 ? '3' : '2',
                // 3 means it should be published, 2 means its pending approval
                'is_active' => $aAd['auto_publish'] == 1 ? '1' : '0'
            ], 'sponsor_id = ' . $aAd['sponsor_id']);

            if (!empty($sModule) && $aAd['auto_publish'] == 1) {
            ...
            $this->database()->update(Phpfox::getT('ad'), [
                'is_custom' => '2'
            ], 'ad_id = ' . $iItemId);
        }
        ...
        $sCallbackFunction = 'getToSponsor' . ucfirst($sSection) . 'Info';
        if (Phpfox::hasCallback($sModule, $sCallbackFunction)) {
            $aReturn = Phpfox::callback($sModule . '.' . $sCallbackFunction, $iItemId);
            $aReturn['status'] = $sStatus;
            $aReturn['return_array'] = true;
            return $aReturn;
        }
    ...
    public function pendingApproval()
    {
        return [
            ...
            'value' => Phpfox::getService('ad')->getPendingSponsorshipsCount(),
            'link' => Phpfox_Url::instance()->makeUrl('admincp.ad.sponsor', ['view' => 'pending'])
        ];
    }

    public function getAdmincpAlertItems()
    {
        $iTotalPending = Phpfox::getService('ad')->getPendingSponsorshipsCount();
        return [
            'message' => _p('you_have_total_pending_sponsorships', ['total' => $iTotalPending]),
            'value' => $iTotalPending,
            'link' => Phpfox_Url::instance()->makeUrl('admincp.ad.sponsor', array('view' => 'pending'))
        ];
    }
    ...
    {
        return [
            'link' => Phpfox_Url::instance()->makeUrl('ad.sponsor', array('view' => $aNotification['item_id'])),
    ...
    {
        return [
            'link' => Phpfox_Url::instance()->makeUrl('ad.manage'),

...
{else}
  ...
  {if Phpfox::isUser() && !defined('PHPFOX_IS_USER_PROFILE') && !PHPFOX_IS_AJAX && !defined('PHPFOX_IS_PAGES_VIEW') && Phpfox::getParam('feed.allow_choose_sort_on_feeds') && empty($aFeedCallback.disable_sort)}
    <div class="feed_sort_order">
      ...
      <div class="feed_sort_holder open" data-action="feed_sort_holder_click">
...
{if count($aFeeds) || (isset($bForceReloadOnPage) && $bForceReloadOnPage)}
  ...
  <div id="feed_view_more">
    {if $bIsHashTagPop}
    {if count($aFeeds) > 8}
    <a href="{url link='hashtag'}{$sIsHashTagSearch}/page_1/" class="global_view_more no_ajax_link" style="display:block;">{_p var='load_more'}</a>
    {/if}
    {else}
    <div id="js_feed_pass_info" style="display:none;">page={$iFeedNextPage}{if defined('PHPFOX_IS_USER_PROFILE') && isset($iUserProfileId)}&profile_user_id={$iUserProfileId}{/if}{if isset($aFeedCallback.module)}&callback_module_id={$aFeedCallback.module}&callback_item_id={$aFeedCallback.item_id}{/if}&year={$sTimelineYear}&month={$sTimelineMonth}{if !empty($sIsHashTagSearch)}&hashtagsearch={$sIsHashTagSearch}{/if}</div>
    <div id="feed_view_more_loader"><i class="fa fa-spin fa-circle-o-notch" aria-hidden="true"></i></div>
    <a {if !PHPFOX_IS_AJAX && !defined('FEED_LOAD_NEW_FEEDS') && !defined('FEED_LOAD_MORE_NEWS') && isset($bForceReloadOnPage) && $bForceReloadOnPage} style="text-indent:-1000px; overflow:hidden; background:transparent; border:0px;"{/if} href="{if Phpfox_Module::instance()->getFullControllerName() == 'core.index-visitor'}{url link='core.index-visitor' page=$iFeedNextPage}{else}{url link='current' page=$iFeedNextPage}{/if}" onclick="$(this).hide(); $('#feed_view_more_loader').show();var oLastFeed = $('.js_parent_feed_entry').last();var iLastFeedId = (oLastFeed) ? oLastFeed.attr('id') : null; $.ajaxCall('feed.viewMore', 'page={$iFeedNextPage}{if defined('PHPFOX_IS_USER_PROFILE') && isset($iUserProfileId)}&profile_user_id={$iUserProfileId}{/if}{if isset($aFeedCallback.module)}&callback_module_id={$aFeedCallback.module}&callback_item_id={$aFeedCallback.item_id}{/if}&year={$sTimelineYear}&month={$sTimelineMonth}{if !empty($sIsHashTagSearch)}&hashtagsearch={$sIsHashTagSearch}{/if}&last-feed-id='+iLastFeedId, 'GET'); return false;" class="global_view_more no_ajax_link">{_p var='load_more'}</a>

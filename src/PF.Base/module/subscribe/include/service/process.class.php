...
class Subscribe_Service_Process extends Phpfox_Service
{
    ...
    public function add($aVals, $iUpdateId = null)
    {
        ...
        foreach ($aLanguages as $aLanguage) {
            if (empty($aVals['title'][$aLanguage['language_code']])) {
                $aVals['title']['language_code'] = $aVals['title'][$sDefaultLanguageCode];
            }
            if (empty($aVals['description'][$aLanguage['language_code']])) {
                $aVals['description']['language_code'] = $aVals['description'][$sDefaultLanguageCode];
            }
        }
        if ($iUpdateId) {
            ...
            if (!$bAlreadyPhrase) {
                ...
                \Core\Lib::phrase()->addPhrase($sDescriptionVarName , $aVals['description'], true);
            }
        } else {
    ...
    public function delete($iId)
    {
        ...
        $aPackage = $this->database()->select('package_id, image_path, server_id, title, description')
            ->from($this->_sTable)
            ->where('package_id = ' . (int)$iId)
            ->execute('getSlaveRow');

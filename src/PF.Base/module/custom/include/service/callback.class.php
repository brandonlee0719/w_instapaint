...
class Custom_Service_Callback extends Phpfox_Service
{
    ...
    public function getNotificationLike($aNotification)
    {
        ...
        if ($aNotification['user_id'] == $aRow['user_id']) {
            $sPhrase = _p('users_liked_gender_own_profile_update_title', array(
                'users' => $sUsers,
    ...
    public function addCommentRelation($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        if (empty($aVals['parent_id'])) {
            ...
            $this->database()->update(Phpfox::getT('custom_relation_data'), array('total_comment' => ($iCount + 1)),
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => _p('name_commented_on_your_profile_update_title',
                    array('name' => $sName, 'title' => $sTitle)),
                'owner_message' => _p('name_commented_on_your_profile_update_a_href_link_content_a',
                    array('name' => $sName, 'link' => $sLink, 'content' => $sContent)),
                'owner_notification' => 'comment.add_new_comment',
                'notify_id' => 'custom_comment_relation',
                'mass_id' => 'custom',
                'mass_subject' => (Phpfox::getUserId() == $aRow['user_id'] ? _p('name_commented_on_gender_profile_update',
                    array('name' => $sName, 'gender' => $sGender)) : _p('name_commented_on_full_name_s_profile_update',
                    array('name' => $sName, 'full_name' => $aRow['full_name']))),
                'mass_message' => (Phpfox::getUserId() == $aRow['user_id'] ? _p('user_commented_on_gender_profile_update_message',
                    array(
                        'name' => $sName,
                        'gender' => $sGender,
                        'link' => $sLink,
                        'content' => $sContent
                    )) : _p('name_commented_on_full_name_s_profile_update_message', array(
                    'name' => $sName,
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify(array(
                ...
                'owner_subject' => _p('full_name_commented_on_your_profile_update_title', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'title' => $this->preParse()->clean(_p($aRow['phrase_var_name']), 100)
                )),
                'owner_message' => _p('full_name_commented_on_your_profile_update_message', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                    'link' => $sLink,
                ...
                'mass_id' => 'custom',
                'mass_subject' => (Phpfox::getUserId() == $aRow['user_id'] ? _p('full_name_commented_on_gender_profile_update',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1)
                    )) : _p('full_name_commented_on_row_full_name_s_video',
                    array('full_name' => Phpfox::getUserBy('full_name'), 'row_full_name' => $aRow['full_name']))),
                'mass_message' => (Phpfox::getUserId() == $aRow['user_id'] ? _p('full_name_commented_on_gender_profile_update_message',
                    array(
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aRow['gender'], 1),
                        'link' => $sLink,
                        'title' => _p($aRow['phrase_var_name'])
                    )) : _p('full_name_commented_on_row_full_name_s_profile_update', array(
                    'full_name' => Phpfox::getUserBy('full_name'),
                ...
                switch ($aVal['user_group_id']) {
                    case 2:
                        $this->database()->update(Phpfox::getT('user_group_setting'),
                            array('is_hidden' => 1, 'default_user' => $sTableName), 'setting_id = ' . $iSettingId);
                        break;
                    case 1:
                        $this->database()->update(Phpfox::getT('user_group_setting'),
                            array('is_hidden' => 1, 'default_admin' => $sTableName), 'setting_id = ' . $iSettingId);
                        break;

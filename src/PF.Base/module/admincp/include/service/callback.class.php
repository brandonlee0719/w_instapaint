...
class Admincp_Service_Callback extends Phpfox_Service
{
    ...
    public function getAdmincpAlertItems()
    {
        ...
        if(false === $aResult or ($aResult['timestamp'] + 1800 < time())){
    ...
    public function updateCounterList()
    {
        ...
        $aList[] = [
            'name' => _p('update_privacy_for_v3_upgrade'),
            'id' => 'update-v3-tables'
        ];

        if (db()->tableExists(Phpfox::getT('group')) && Phpfox::isModule('pages')) {
            $aList[] = [
                'name' => _p('import_groups_from_v2_to_v3_pages'),
                'id' => 'import-groups'
            ];
        }
    ...
    public function updateCounter($iId, $iPage, $iPageLimit)
    {
        ...
        if ($iId == 'import-groups' && Phpfox::isModule('pages') && db()->tableExists(Phpfox::getT('group'))) {

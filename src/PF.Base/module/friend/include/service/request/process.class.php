...
class Friend_Service_Request_Process extends Phpfox_Service 
{
	...
	{		
        ...
        Phpfox::getLib('cache')->remove('friend_build_cache_data_' . $iFriendId);
        Phpfox::getLib('cache')->remove('friend_build_cache_data_' . $iUserId);
    ...
    public function delete($iId, $iUserId)
    {
        $this->database()->delete($this->_sTable,
            'request_id = ' . (int)$iId . ' AND friend_user_id = ' . (int)$iUserId);
        Phpfox::getLib('cache')->remove('friend_build_cache_data_' . $iId);
        Phpfox::getLib('cache')->remove('friend_build_cache_data_' . $iUserId);
        return true;
    }

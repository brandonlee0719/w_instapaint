...
class Friend_Service_Friend extends Phpfox_Service
{
	...
	public function getFromCache($mAllowCustom = false, $sUserSearch = false)
	{
        ...
        $sCacheId = Phpfox::getLib('cache')->set('friend_build_cache_data_' . Phpfox::getUserId());

        $aRows = Phpfox::getLib('cache')->get($sCacheId);

        if ($aRows === false) {
            if ($sUserSearch != false) {
                $this->database()->join($this->_sTable, 'f',
                    'u.user_id = f.friend_user_id AND f.user_id=' . Phpfox::getUserId());
            ...
            foreach ($aRows as $iKey => $aRow) {
                ...
                if (Phpfox::getParam('mail.disallow_select_of_recipients')) {
                    if (!Phpfox::getService('mail')->canMessageUser($aRow['user_id'])) {
                        unset($aRows[$iKey]);
                        continue;
                    }
                }
                ...
                $aRows[$iKey]['user_profile'] = ($aRow['profile_page_id'] ? Phpfox::getService('pages')->getUrl($aRow['profile_page_id'],
                    '', $aRow['user_name']) : Phpfox_Url::instance()->makeUrl($aRow['user_name']));
                $aRows[$iKey]['is_page'] = ($aRow['profile_page_id'] ? true : false);
                $aRows[$iKey]['user_image'] = Phpfox::getLib('image.helper')->display([
                    'user' => $aRow,
                    'suffix' => '_50_square',
                    'max_height' => 32,
                    'max_width' => 32,
                ...
                $aRows[$iKey]['user_image_actual'] = Phpfox::getLib('image.helper')->display([
                    'user' => $aRow,
                    'suffix' => '_50_square',
                    'max_height' => 32,
                    'max_width' => 32

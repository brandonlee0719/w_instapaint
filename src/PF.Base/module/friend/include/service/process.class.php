...
class Friend_Service_Process extends Phpfox_Service 
{
	...
	public function add($iUserId, $iFriendId, $iFolderId = 0)
	{
		...
		// Add to feed
		$feed_id = (Phpfox::isModule('feed') ? Phpfox::getService('feed.process')->add('friend', $iFriendToUser, 0, 0, $iFriendId, Phpfox::getUserId()) : false);
	...
	public function delete($iId, $bIsFriendId = true)
	{
        ...
        //remove friends list cache
        $this->cache()->remove('friend_build_cache_data_' . Phpfox::getUserId());
        $this->cache()->remove('friend_build_cache_data_' . $aFriend['friend_user_id']);

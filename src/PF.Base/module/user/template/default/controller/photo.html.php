	...
	{plugin call='user.template_controller_photo_1'}
		...
		<div id="js_crop_tool" class="block dont-unbind-children">
			{if ($bIsProcess) || (!$bIsProcess && $iImageWidth > $iMinWidth && $iImageHeight > $iMinWidth)}
				<div class="title">{_p var='profile_picture_cropping_tool'}</div>
				<div class="content">
				{if ($bIsProcess && ($iImageWidth <= $iMinWidth || $iImageHeight <= $iMinWidth))}
					<div class="extra_info">
            <span id="no_crop_user_avatar" data-id="1">
						{_p var='the_image_you_uploaded_is_rather_small_so_we_are_unable_to_crop_it_however_you_can_still_save_this_image_if_you_want_to_use_it'}
            </span>
					</div>	
					<form method="post" action="{url link='current'}">
						...
						<input type="submit" value="{_p var='save_avatar'}" id="js_save_profile_photo" class="button btn-primary" /> <span id="js_photo_preview_ajax"></span>
					</form>
				{else}
					<div class="extra_info">
						{plugin call='user.template_controller_photo_4'}
						{_p var='click_and_drag_a_box_on_the_original_image_to_create_your_cropped_image'}
					</div>
					<div class="p_4">
						...
						<form method="post" action="{url link='current'}" class="form">
							...
							<div style="margin-top:10px;">
								<input type="submit" value="{_p var='save_avatar'}" id="js_save_profile_photo" class="btn btn-primary" /> <span id="js_photo_preview_ajax"></span>
							</div>
						</form>			
					</div>
				{/if}
...
<div class="block">
	...
	<div class="title">{_p var='select_an_image'}</div>
	{/if}

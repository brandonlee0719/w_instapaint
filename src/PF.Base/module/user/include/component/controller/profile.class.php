...
class User_Component_Controller_Profile extends Phpfox_Component
{
	...
	public function process()
	{
		...
		foreach ($aCustomGroups as $iKey => &$aCustomGroup)
		{
			...
			$sPharse = _p($aCustomGroup['phrase_var_name']);
            if (strlen($sPharse) > 30) {
                $aCustomGroup['phrase_var_name'] = substr($sPharse, 0, 30);
                $aCustomGroup['phrase_var_name'] .= '...';
            } else {
                $aCustomGroup['phrase_var_name'] = $sPharse;
            }
        }

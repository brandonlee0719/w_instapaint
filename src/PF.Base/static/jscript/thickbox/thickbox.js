...
function js_box_remove($oObj) {
  ...
  boxImageHolder.fadeOut(100, function() {
    $(this).remove();
    if ($('.js_box_image_holder, .js_box_holder').length == 0) {
      $('body').css('overflow', 'auto');
    }
  });
  boxHolder.fadeOut(100, function() {
    $(this).remove();
    if ($('.js_box_image_holder, .js_box_holder').not('#profile_photo_form').length == 0) {
      $('body').css('overflow', 'auto');
    }
  });
...
function tb_show(
    caption, url, thisObject, sForceMessage, bForceNoClick, sType, hidden, holderId) {
  ...
  if (urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' ||
    ...
    imgPreloader.onload = function() {
      ...
      if ($(thisObject).parents('.js_box_thumbs_holder').length > 0) {
        ...
        sHtml += '<div>';
        $(thisObject).
            parents('.js_box_thumbs_holder').
            find('.js_box_image_holder_thumbs').
            find('.thickbox').
            each(function() {
    ...
    if (params['type']) {
      switch (params['type']) {
        case 'delete':
          ...
          if (!params['call']) {
            sHtml += ' <input class="btn" type="submit" value="' +
                getPhrase('yes') + '" class="btn" /> ';
          }
          else {
            sHtml += ' <input class="btn" type="button" value="' +
                getPhrase('yes') +
                '" class="btn" onclick="$(\'#js_inline_delete_id\').ajaxCall(\'' +
                params['call'] + '\', \'' + queryString +
                '\'); tb_remove();" /> ';
          }
          sHtml += ' <input class="btn btn-success" type="button" value="' +
      ...
      $.ajax({
        ...
        success: function(e) {
          ...
          if (typeof(e.error) == 'string' && typeof(e.content) != 'string') {
            $oNew.find('.js_box_content').
                html('<div class="error_message">' + e.error + '</div>');
            return;
          }
          ...
          $oNew.find('.js_box_title:first').
              html(((e.h1_clean && e.h1_clean.length)
                  ? e.h1_clean
                  : e.actual_title));
          $oNew.find('.js_box_content').html('');
    ...
    $.ajax(
        {
          ...
          success: function(sMsg) {
            ...
            if (!empty($oNew.find('.js_box_title_store:first').html())) {
              $oNew.find('.js_box_title:first').
                  html($oNew.find('.js_box_title_store:first').html());
            }
            $oNew.find('.js_box_title:first').show();
            $oNew.find('.js_box_close:first').show();
...
function tb_remove() {
  ...
  if (isset($aAllBoxIndex[$iFocusBox]) &&
      $('#' + $aAllBoxIndex[$iFocusBox]).length > 0) {
    var $sLink = $('#' + $aAllBoxIndex[$iFocusBox]).
        find('.js_box_history:first').
        html();

    $('#' + $aAllBoxIndex[$iFocusBox]).
        fadeOut('100', function() {$(this).remove();});
    $('#' + $aAllBoxIndex[$iFocusBox]).
        parent('.js_box_image_holder').
        fadeOut('100', function() {$(this).remove();});
  ...
  $('.js_box_holder').fadeOut(100, function() {
    $(this).remove();
    if ($('.js_box_image_holder, .js_box_holder').length == 0) {
      $('body').css('overflow', 'auto');
    }
